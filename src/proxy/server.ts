...
import { getRouter, type RoutingDecision } from '../router/index.js';
...
// UsageLog type imported from storage/database.js
...
export function createProxy(config: Partial<ProxyConfig> = {}, storage?: SQLiteAdapter) {
  const app = new Hono();
  ...
  app.post('/v1/messages', async (c) => {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    const router = getRouter(store);
    const routingDecision: RoutingDecision = router.route(body.model);
    ...
    if (response.status === 429) {
      ...
      const windowLogs = store.getWindowLogs(5);
      const tokensUsed = windowLogs.reduce(
...
export async function startProxy(port: number = 4141) {
