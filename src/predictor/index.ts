...
export interface VelocityStats {
  ...
  trend: number[]; // Last N measurements for sparkline (12 data points)
...
export interface Prediction {
  ...
  minutesUntilLimit: number; // Time until predicted rate limit
  tokensRemaining: number;
  confidence: number; // 0-100
  velocity: VelocityStats;
  recommendedAction: 'continue' | 'switch_model' | 'switch_provider' | 'pause';
  estimatedLimitTime?: Date; // When limit will be hit
}

export interface LearnedLimit {
  model: string;
  tokenLimit: number; // Learned from 429 errors
  windowHours: number; // Time window for limit (typically 5 hours)
  confidence: number; // 0-100, increases with data points
  lastUpdated: Date;
  dataPoints: number; // Number of 429 errors observed
}
...
export class RateLimitTracker {
  ...
  private learnedLimits: Map<string, LearnedLimit>;
   ...
   */
  private loadLearnedLimits(): void {
    ...
    const eventsByModel = events.reduce((acc, event) => {
      ...
      return acc;
    }, {} as Record<string, RateLimitEvent[]>);
    ...
    for (const [model, modelEvents] of Object.entries(eventsByModel)) {
      ...
      const avgTokens =
        modelEvents.reduce((sum, e) => sum + e.tokensUsedBeforeLimit, 0) / modelEvents.length;
  ...
  recordRateLimit(event: Omit<RateLimitEvent, 'id'>): void {
    ...
    if (existing) {
      ...
      const newTokenLimit = Math.floor(
        (existing.tokenLimit * existing.dataPoints + event.tokensUsedBeforeLimit * 0.95) /
          newDataPoints
      );
...
export class VelocityCalculator {
   ...
   */
  calculate(logs: UsageLog[], windowMinutes: number = 60): VelocityStats {
    if (logs.length === 0) {
      return {
        tokensPerHour: 0,
        tokensPerMinute: 0,
   ...
   */
  private calculateTrend(logs: UsageLog[], points: number): number[] {
    if (logs.length === 0) return new Array(points).fill(0);
   ...
   */
  private calculateAcceleration(trend: number[]): number {
...
export class PredictionEngine {
  private storage: SQLiteAdapter;
  private limitsTracker: RateLimitTracker;
   ...
   */
  predict(windowHours: number = 5, model: string = 'claude-sonnet-4-20250514'): Prediction {
    ...
    const currentUsage = logs.reduce(
      (sum, log) => sum + log.inputTokens + log.outputTokens,
      0
    );
    ...
    let estimatedLimitTime: Date | undefined;
   ...
   */
  getPredictionAccuracy(): { predicted: number; actual: number; accuracy: number }[] {
...
export function getPredictionEngine(storage: SQLiteAdapter): PredictionEngine {
  if (!instance) {
